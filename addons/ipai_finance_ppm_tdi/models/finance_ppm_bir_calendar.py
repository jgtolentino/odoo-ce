...
class FinancePPMBIRCalendar(models.Model):
    ...
    form_code = fields.Char(
        string="Form Code", required=True, help="BIR form code (e.g., 1601-C, 2550Q)"
    )
    form_name = fields.Char(
        string="Form Name", required=True, help="Official BIR form name"
    )
    period = fields.Char(
        string="Period",
        required=True,
    ...
    filing_deadline = fields.Date(
        string="Filing Deadline", required=True, help="Official BIR filing deadline"
    )
    ...
    description = fields.Text(
        string="Description", help="Detailed description of form purpose and coverage"
    )

    _sql_constraints = [
        (
            "unique_form_period",
            "UNIQUE(form_code, period)",
